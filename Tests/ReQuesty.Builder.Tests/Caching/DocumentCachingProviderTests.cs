...
namespace ReQuesty.Builder.Caching.Tests;
...
public class DocumentCachingProviderTests
{
    ...
    public async Task DefensiveProgrammingAsync()
    {
        ...
        DocumentCachingProvider provider = new(client, mockLogger);
  ...
  ""headers"": {
    ...
    {
        HttpClient client = HttpClientInstance.Value; //not disposed on purpose
        ILogger mockLogger = new Mock<ILogger>().Object;
        DocumentCachingProvider provider = new(client, mockLogger);
        await using Stream result = await provider.GetDocumentAsync(new Uri("https://localhost/foo.json"), "foo", "bar.json");
    ...
    public async Task GetsWhenCacheIsOutdatedAsync()
    {
        ...
        ILogger mockLogger = new Mock<ILogger>().Object;
        DocumentCachingProvider provider = new(client, mockLogger);
        await using Stream result1 = await provider.GetDocumentAsync(new Uri("https://localhost/foo.json"), "foo", "bar.json");
        provider.Duration = TimeSpan.FromMilliseconds(-1);
        await using Stream result2 = await provider.GetDocumentAsync(new Uri("https://localhost/foo.json"), "foo", "bar.json");
