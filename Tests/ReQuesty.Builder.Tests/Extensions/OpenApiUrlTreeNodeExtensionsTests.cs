...
public sealed class OpenApiUrlTreeNodeExtensionsTests : IDisposable
{
    ...
    public void IsComplexPathWithAnyNumberOfParameters()
    {
        ...
        OpenApiUrlTreeNode node = OpenApiUrlTreeNode.Create(doc, Label);
    ...
    public void GetsClassNameWithIndexerAndExtension()
    {
        ...
        OpenApiUrlTreeNode node = OpenApiUrlTreeNode.Create(doc, Label);
        string result = node.Children["reviews"].Children["{resource-type}.json"].GetClassName(["application/json"]);
    ...
    public void GetsClassNameWithSegmentsToSkipForClassNames()
    {
        ...
        IOpenApiSchema? responseSchema = node.Children["reviews"].Children["{resource-type}.json"].PathItems["default"].Operations![HttpMethod.Get].Responses!["200"].Content!["application/json"].Schema;
        string responseClassName = node.Children["reviews"].Children["{resource-type}.json"]
            .GetClassName(["application/json"], schema: responseSchema);
    ...
    public void SinglePathParametersAreDeduplicated()
    {
        ...
        OpenApiUrlTreeNode? usersCollectionIndexNode = GetChildNodeByPath(node, "users/{foo-id}");
        Assert.NotNull(usersCollectionIndexNode);
        Assert.Equal("{+baseurl}/users/{foo%2Did}", usersCollectionIndexNode.GetUrlTemplate());

        OpenApiUrlTreeNode? managerNode = GetChildNodeByPath(node, "users/{foo-id}/manager");
        Assert.NotNull(managerNode);
        Assert.Equal("{+baseurl}/users/{foo%2Did}/manager", managerNode.GetUrlTemplate());

        OpenApiUrlTreeNode? careerAdvisorNode = GetChildNodeByPath(node, "users/{foo-id}/careerAdvisor");
        Assert.NotNull(careerAdvisorNode);
        Assert.Equal("{+baseurl}/users/{foo%2Did}/careerAdvisor", careerAdvisorNode.GetUrlTemplate());

        OpenApiUrlTreeNode? careerAdvisorIndexNode = GetChildNodeByPath(node, "users/{foo-id}/careerAdvisor/{id}");
        ...
        IOpenApiPathItem pathItem = careerAdvisorIndexNode.PathItems[Constants.DefaultOpenApiLabel];
        Assert.NotNull(pathItem);
        IOpenApiParameter? parameter = pathItem.Parameters!.FirstOrDefault(static p => p.Name == "foo-id");
        Assert.NotNull(parameter);
    }
    [Fact]
    public void SinglePathParametersAreDeduplicatedAndOrderIsRespected()
    {
        OpenApiSchema ownerSchema = new()
        {
            Type = JsonSchemaType.Object,
            Properties = new Dictionary<string, IOpenApiSchema> {
                {
        ...
        document.SetReferenceHostDocument();
        CountLogger<ReQuestyBuilder> mockLogger = new();
        ReQuestyBuilder builder = new(mockLogger, new GenerationConfiguration { ClientClassName = "GitHub", ApiRootUrl = "https://localhost" }, _httpClient);
        OpenApiUrlTreeNode node = builder.CreateUriSpace(document);
        node.MergeIndexNodesAtSameLevel(mockLogger);

        // Expected
        OpenApiUrlTreeNode? resultNode = GetChildNodeByPath(node, "repos/{owner-id}/{repo-id}/generate");
    ...
    public void repro4085()
    {
        ...
        // Expected
        OpenApiUrlTreeNode? resultNode = GetChildNodeByPath(node, "path");
        Assert.NotNull(resultNode);
        ...
        OpenApiUrlTreeNode? differentThingId = GetChildNodeByPath(resultNode, "{differentThing-id}");
    ...
    {
        string result = OpenApiUrlTreeNodeExtensions.CleanupParametersFromPath(segmentName);
    ...
    private static OpenApiUrlTreeNode? GetChildNodeByPath(OpenApiUrlTreeNode node, string path)
    {
        ...
        if (pathSegments.Length == 1 && node.Children.TryGetValue(pathSegments[0], out OpenApiUrlTreeNode? result))
        {
            return result;
        }

        if (node.Children.TryGetValue(pathSegments[0], out OpenApiUrlTreeNode? currentNode))
        {
