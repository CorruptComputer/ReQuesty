...
public static class OpenApiOperationExtensions
{
    ...
    {
        int slashIndex = input.IndexOf('/', StringComparison.OrdinalIgnoreCase);
        int plusIndex = input.IndexOf('+', StringComparison.OrdinalIgnoreCase);
    ...
    internal static bool IsMultipartTopMimeType(this IDictionary<string, OpenApiMediaType> source, StructuredMimeTypesCollection structuredMimeTypes)
    {
        ...
        if (!source.ContainsKey(multipartMimeTypes.First()))
        {
            return false;
        }

        if (source.Count == 1 || !source.Keys.Where(static x => !multipartMimeTypes.Contains(x)).Any(structuredMimeTypes.Contains))
        {
            return true;
        }
