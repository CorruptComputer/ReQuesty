...
public static partial class OpenApiUrlTreeNodeExtensions
{
    ...
    {
        ArgumentNullException.ThrowIfNull(currentNode);
        string result = currentNode.GetSegmentName(structuredMimeTypes, suffix, prefix, operation, response, schema, requestBody, static x => string.Join(string.Empty, x.Select(static (y, idx) => idx == 0 ? y : y.ToFirstCharacterUpperCase())), false, placeholder);
    ...
    {
        string? referenceName = schema?.GetClassName();
        string? rawClassName = referenceName is not null && !string.IsNullOrEmpty(referenceName) ?
                            referenceName :
        ...
        IEnumerable<string> classNameSegments = rawClassName?.Split('.', StringSplitOptions.RemoveEmptyEntries).AsEnumerable() ?? [];
    ...
    internal static bool IsPathSegmentWithSingleSimpleParameter(this string currentSegment)
    {
        ...
        string segmentWithoutExtension = stripExtensionForIndexersRegex().Replace(currentSegment, string.Empty);
    ...
    {
        ArgumentNullException.ThrowIfNull(currentNode);
        if (!currentNode.PathItems.TryGetValue(Constants.DefaultOpenApiLabel, out IOpenApiPathItem? pathItem))
        {
            return false;
        }

        IEnumerable<IOpenApiParameter> operationQueryParameters = pathItem.Operations?.SelectMany(static x => x.Value.Parameters ?? Enumerable.Empty<IOpenApiParameter>()) ?? [];
    ...
    {
        IEnumerable<IOpenApiParameter> operationQueryParameters = (operationType, pathItem.Operations is { Count: > 0 }) switch
        {
            (HttpMethod ot, _) when pathItem.Operations!.TryGetValue(ot, out OpenApiOperation? operation) && operation.Parameters is not null => operation.Parameters,
    ...
    public static string GetUrlTemplate(this OpenApiUrlTreeNode currentNode, HttpMethod? operationType = null, bool includeQueryParameters = true, bool includeBaseUrl = true)
    {
        ...
        string trailingSlashItem = $"{currentNode.Path}\\";
        if (includeQueryParameters && currentNode.HasOperations(Constants.DefaultOpenApiLabel))
        {
            IOpenApiPathItem pathItem = currentNode.PathItems[Constants.DefaultOpenApiLabel];

            IOpenApiParameter[] parameters = pathItem.GetParameters(operationType);
            if (parameters.Length != 0)
            {
                string requiredParameters = string.Join("&", parameters.Where(static x => x.Required)
                                                .Select(static x =>
                                                            $"{x.Name}={{{x.Name?.SanitizeParameterNameForUrlTemplate()}}}"));
                string optionalParameters = string.Join(",", parameters.Where(static x => !x.Required)
                                                .Select(static x =>
                                                            x.Name?.SanitizeParameterNameForUrlTemplate() +
                                                            (x.Explode ?
                                                                "*" : string.Empty)));
                bool hasRequiredParameters = !string.IsNullOrEmpty(requiredParameters);
                bool hasOptionalParameters = !string.IsNullOrEmpty(optionalParameters);
        ...
        HashSet<string> pathReservedPathParametersIds = currentNode.PathItems.TryGetValue(Constants.DefaultOpenApiLabel, out IOpenApiPathItem? pItem) ?
                                                pItem.Parameters?
                                                        .Union(pItem.Operations?.SelectMany(static x => x.Value.Parameters ?? Enumerable.Empty<IOpenApiParameter>()) ?? [])
                                                        .Where(static x => x.In == ParameterLocation.Path && x.Extensions is not null && x.Extensions.TryGetValue(OpenApiReservedParameterExtension.Name, out IOpenApiExtension? ext) && ext is OpenApiReservedParameterExtension reserved && reserved.IsReserved.HasValue && reserved.IsReserved.Value)
    ...
    {
        foreach (KeyValuePair<string, HashSet<string>> childInfo in currentNode.Children.Values.SelectMany(static x => x.GetRequestInfoInternal()))
        {
    ...
    {
        if (string.IsNullOrEmpty(original) || !original.Contains('{', StringComparison.OrdinalIgnoreCase))
        {
            return original;
        }

        string updated = original;
        foreach (ValueMatch match in pathParamMatcher().EnumerateMatches(original))
        {
            string value = original[(match.Index + 1)..(match.Index + match.Length - 1)];// ignore the { and }
    ...
    private const string DeduplicatedSegmentKey = "x-requesty-deduplicatedSegment";
    public static string DeduplicatedSegment(this OpenApiUrlTreeNode currentNode)
    {
        ...
        if (currentNode.AdditionalData.TryGetValue(DeduplicatedSegmentKey, out List<string>? deduplicatedSegment) && deduplicatedSegment.FirstOrDefault() is string deduplicatedSegmentString)
        {
    ...
    {
        KeyValuePair<string, OpenApiUrlTreeNode>[] indexNodes = node.Children
        ...
        {
            KeyValuePair<string, OpenApiUrlTreeNode> indexNode = indexNodes[0];
            node.Children.Remove(indexNode.Key);
            string oldSegmentName = indexNode.Value.Segment.Trim('{', '}').CleanupSymbolName();
            int segmentIndex = indexNode.Value.Path.Split('\\', StringSplitOptions.RemoveEmptyEntries).ToList().IndexOf(indexNode.Value.Segment);
            string newSegmentParameterName = oldSegmentName.EndsWith("-id", StringComparison.OrdinalIgnoreCase) ? oldSegmentName : $"{{{oldSegmentName.TrimSuffix("id", StringComparison.OrdinalIgnoreCase)}-id}}";
            ...
            indexNode.Value.AdditionalData.Add(Constants.ReQuestySegmentNameTreeNodeExtensionKey, [newSegmentParameterName]);
            ...
            foreach (KeyValuePair<string, OpenApiUrlTreeNode> child in indexNodes.Except([indexNode]))
            {
        ...
        foreach (OpenApiUrlTreeNode child in node.Children.Values)
        {
    ...
    private static void ReplaceParameterInPathForAllChildNodes(OpenApiUrlTreeNode node, int parameterIndex, string newParameterName)
    {
        ...
        foreach (OpenApiUrlTreeNode child in node.Children.Values)
        {
            string[] splatPath = child.Path.Split('\\', StringSplitOptions.RemoveEmptyEntries);
            if (splatPath.Length > parameterIndex)
            {
                string oldName = splatPath[parameterIndex];
                splatPath[parameterIndex] = newParameterName;
                child.Path = "\\" + string.Join('\\', splatPath);
                if (node.PathItems.TryGetValue(Constants.DefaultOpenApiLabel, out IOpenApiPathItem? pathItem) && pathItem.Parameters is not null)
                {
                    foreach (IOpenApiParameter? pathParameter in pathItem.Parameters
                                                    .Union(pathItem.Operations?.SelectMany(static x => x.Value.Parameters ?? Enumerable.Empty<IOpenApiParameter>()) ?? [])
    ...
    {
        foreach (KeyValuePair<string, OpenApiUrlTreeNode> child in source.Children)
        {
        ...
        foreach (KeyValuePair<string, IOpenApiPathItem> pathItem in source.PathItems)
        {
            foreach (IOpenApiParameter? pathParameter in (pathItem.Value.Parameters ?? Enumerable.Empty<IOpenApiParameter>())
            ...
            if (source != destination && !destination.PathItems.TryAdd(pathItem.Key, pathItem.Value) &&
                destination.PathItems.TryGetValue(pathItem.Key, out IOpenApiPathItem? dpi) && dpi is OpenApiPathItem destinationPathItem)
            {
                if (pathItem.Value.Operations is { Count: > 0 })
                {
                    destinationPathItem.Operations ??= [];
                    foreach (KeyValuePair<HttpMethod, OpenApiOperation> operation in pathItem.Value.Operations)
                    {
                ...
                {
                    destinationPathItem.Parameters ??= [];
                    foreach (IOpenApiParameter pathParameter in pathItem.Value.Parameters)
                    {
                ...
                if (pathItem.Value.Extensions is { Count: > 0 })
                {
                    ...
                    foreach (KeyValuePair<string, IOpenApiExtension> extension in pathItem.Value.Extensions)
                    {
