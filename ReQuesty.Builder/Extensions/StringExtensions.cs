...
public static partial class StringExtensions
{
    ...
    private static string ToInternalCamelCase(string? input, char[] separators, bool firstCharacterUpperCase = false, bool normalizeFirstCharacter = true)
    {
        ...
        string[] chunks = input.Split(separators, StringSplitOptions.RemoveEmptyEntries);
        if (chunks.Length == 0)
        {
            return string.Empty;
        }
    ...
    public static string EscapeSuffix(this string? name, HashSet<string> specialFileNameSuffixes, char separator = '_')
    {
        ...
        string last = name.Split(separator)[^1];
    ...
    public static string ToSnakeCase(this string? name, char separator = '_')
    {
        ...
        int indexOfLess = nameSpan.IndexOf('<');
        if (indexOfLess >= 0)
        {
        ...
        int newStringLength = nameSpan.Length + CountNecessaryNewSeparators(nameSpan);
    ...
    private static string HashString(string? input)
    {
        ...
        byte[] hash = (sha.Value ?? throw new InvalidOperationException("unable to get hash algorithm")).ComputeHash(Encoding.UTF8.GetBytes(input));
    ...
    public static string CleanupSymbolName(this string? original)
    {
        ...
        if (result.Length != 0 && int.TryParse(result.AsSpan(0, 1), out int _)) // in most languages a number or starting with a number is not a valid symbol name
        {
