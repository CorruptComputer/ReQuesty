...
public static class OpenApiSchemaExtensions
{
    ...
    public static bool IsInherited(this IOpenApiSchema? schema)
    {
        ...
        IOpenApiSchema[] meaningfulMemberSchemas = schema.AllOf.FlattenSchemaIfRequired(static x => x.AllOf)
                                                                ...
                                                                .ToArray();
        bool isRootSchemaMeaningful = schema.IsSemanticallyMeaningful(ignoreEnums: true, ignoreArrays: true, ignoreType: true);
    ...
    {
        if (schema is null || !schema.IsInclusiveUnion(0))
        {
            return null;
        }

        OpenApiSchema result = schema.GetSchemaOrTargetShallowCopy();
        if (result.AnyOf is null || schema.AnyOf is null)
        {
            return result;
        }
    ...
    {
        if (schema is null || !schema.IsExclusiveUnion(0))
        {
            return null;
        }

        OpenApiSchema result = schema.GetSchemaOrTargetShallowCopy();
        if (result.OneOf is null || schema.OneOf is null)
        {
            return result;
        }
    ...
    internal static IOpenApiSchema? MergeIntersectionSchemaEntries(this IOpenApiSchema? schema, HashSet<IOpenApiSchema>? schemasToExclude = default, bool overrideIntersection = false, Func<IOpenApiSchema, bool>? filter = default)
    {
        ...
        if (schema.AllOf is null || !schema.IsIntersection() && !overrideIntersection)
        {
            return schema;
        }
        ...
        IOpenApiSchema[] meaningfulSchemas = schema.AllOf
                                    ...
                                    .ToArray();
        IOpenApiSchema[] entriesToMerge = meaningfulSchemas.FlattenEmptyEntries(static x => x.AllOf).Union(meaningfulSchemas).ToArray();
    ...
    {
        if (schema is not OpenApiSchemaReference schemaReference || string.IsNullOrEmpty(schemaReference.Reference.Id))
        {
            return;
        }

        result.Extensions ??= new Dictionary<string, IOpenApiExtension>(StringComparer.Ordinal);
        result.Extensions.TryAdd(OpenApiReQuestyMergedExtension.Name, new OpenApiReQuestyMergedExtension(schemaReference.Reference.Id));
    }

    internal static string? GetMergedSchemaOriginalReferenceId(this IOpenApiSchema schema)
    {
        return schema.Extensions is not null && schema.Extensions.TryGetValue(OpenApiReQuestyMergedExtension.Name, out IOpenApiExtension? extension) && extension is OpenApiReQuestyMergedExtension mergedExtension ?
    ...
    internal static void TryAddProperties(this OpenApiSchema schema, IEnumerable<KeyValuePair<string, IOpenApiSchema>> properties)
    {
        ...
        foreach (KeyValuePair<string, IOpenApiSchema> property in properties)
        {
    ...
    {
        IOpenApiSchema[]? meaningfulSchemas = schema?.AllOf?.Where(static x => x.IsSemanticallyMeaningful()).ToArray();
    ...
    public static IEnumerable<string> GetSchemaReferenceIds(this IOpenApiSchema schema, HashSet<IOpenApiSchema>? visitedSchemas = null)
    {
        ...
        if (schema is not null && !visitedSchemas.Contains(schema))
        {
            ...
            List<string> subSchemaReferences = (schema.Properties?.Values ?? Enumerable.Empty<IOpenApiSchema>())
    ...
    private static IEnumerable<IOpenApiSchema> FlattenEmptyEntries(this IEnumerable<IOpenApiSchema> schemas, Func<IOpenApiSchema, IList<IOpenApiSchema>?> subsequentGetter, int? maxDepth = default)
    {
        ...
        List<IOpenApiSchema> result = schemas.ToList();
        Dictionary<IOpenApiSchema, IEnumerable<IOpenApiSchema>> permutations = [];
        foreach (IOpenApiSchema? item in result)
        {
            IList<IOpenApiSchema>? subsequentItems = subsequentGetter(item);
        ...
        {
            foreach (KeyValuePair<IOpenApiSchema, IEnumerable<IOpenApiSchema>> permutation in permutations)
            {
                int index = result.IndexOf(permutation.Key);
                result.RemoveAt(index);
                int offset = 0;
                foreach (IOpenApiSchema insertee in permutation.Value)
                {
    ...
    private static IEnumerable<string> GetAllInheritanceSchemaReferences(string currentReferenceId, ConcurrentDictionary<string, ConcurrentDictionary<string, bool>> inheritanceIndex)
    {
        ...
        if (inheritanceIndex.TryGetValue(currentReferenceId, out ConcurrentDictionary<string, bool>? dependents))
        {
