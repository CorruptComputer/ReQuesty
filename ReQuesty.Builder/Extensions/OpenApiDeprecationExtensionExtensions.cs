...
internal static class OpenApiDeprecationExtensionExtensions
{
    ...
    {
        if (schema.Deprecated && schema.Extensions is not null && schema.Extensions.TryGetValue(OpenApiDeprecationExtension.Name, out IOpenApiExtension? deprecatedExtension) && deprecatedExtension is OpenApiDeprecationExtension deprecatedValue)
        {
    ...
    {
        if (parameter.Deprecated && parameter.Extensions is not null && parameter.Extensions.TryGetValue(OpenApiDeprecationExtension.Name, out IOpenApiExtension? deprecatedExtension) && deprecatedExtension is OpenApiDeprecationExtension deprecatedValue)
        {
            return deprecatedValue.ToDeprecationInformation();
        }
    ...
    {
        if (operation.Deprecated && operation.Extensions is not null && operation.Extensions.TryGetValue(OpenApiDeprecationExtension.Name, out IOpenApiExtension? deprecatedExtension) && deprecatedExtension is OpenApiDeprecationExtension deprecatedValue)
        {
            return deprecatedValue.ToDeprecationInformation();
        }
    ...
    {
        OpenApiOperation[] operations = treeNode.PathItems.TryGetValue(Constants.DefaultOpenApiLabel, out IOpenApiPathItem? pathItem) ? (pathItem.Operations?.Values.ToArray() ?? []) : [];
