...
public class OpenApiAiCapabilitiesExtension : IOpenApiExtension
{
    ...
    public static OpenApiAiCapabilitiesExtension Parse(JsonNode source)
    {
        ...
        OpenApiAiCapabilitiesExtension extension = new();
        if (rawObject.TryGetPropertyValue(nameof(Confirmation).ToFirstCharacterLowerCase(), out JsonNode? confirmation) && confirmation is JsonObject confirmationObject)
        {
            extension.Confirmation = ParseConfirmation(confirmationObject);
        }
        if (rawObject.TryGetPropertyValue(nameof(ResponseSemantics).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? responseSemantics) && responseSemantics is JsonObject responseSemanticsObject)
        {
            extension.ResponseSemantics = ParseResponseSemantics(responseSemanticsObject);
        }
        if (rawObject.TryGetPropertyValue(nameof(SecurityInfo).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? securityInfo) && securityInfo is JsonObject securityInfoObject)
        {
    ...
    {
        ExtensionConfirmation confirmation = new();
        if (source.TryGetPropertyValue(nameof(Confirmation.Type).ToFirstCharacterLowerCase(), out JsonNode? type) &&
            type is JsonValue typeValue && typeValue.GetValueKind() is JsonValueKind.String &&
            typeValue.TryGetValue<string>(out string? typeStrValue))
        {
            confirmation.Type = typeStrValue;
        }

        if (source.TryGetPropertyValue(nameof(Confirmation.Title).ToFirstCharacterLowerCase(), out JsonNode? title) &&
            title is JsonValue titleValue && titleValue.GetValueKind() is JsonValueKind.String &&
            titleValue.TryGetValue<string>(out string? titleStrValue))
        {
            confirmation.Title = titleStrValue;
        }

        if (source.TryGetPropertyValue(nameof(Confirmation.Body).ToFirstCharacterLowerCase(), out JsonNode? body) &&
            body is JsonValue bodyValue && bodyValue.GetValueKind() is JsonValueKind.String &&
            bodyValue.TryGetValue<string>(out string? bodyStrValue))
        {
    ...
    {
        ExtensionResponseSemantics responseSemantics = new();

        if (source.TryGetPropertyValue(nameof(ResponseSemantics.DataPath).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? dataPath) &&
            dataPath is JsonValue dataPathValue && dataPathValue.GetValueKind() is JsonValueKind.String &&
            dataPathValue.TryGetValue<string>(out string? dataPathStrValue))
        {
            responseSemantics.DataPath = dataPathStrValue;
        }
        if (source.TryGetPropertyValue(nameof(ResponseSemantics.StaticTemplate).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? staticTemplate) &&
            staticTemplate is JsonObject staticTemplateObj)
        {
            responseSemantics.StaticTemplate = staticTemplateObj;
        }

        if (source.TryGetPropertyValue(nameof(ResponseSemantics.Properties).ToFirstCharacterLowerCase(), out JsonNode? properties) &&
            properties is JsonObject propertiesObj)
        {
            responseSemantics.Properties = ParseResponseSemanticsProperties(propertiesObj);
        }

        if (source.TryGetPropertyValue(nameof(ResponseSemantics.OauthCardPath).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? OauthCardPath) &&
            OauthCardPath is JsonValue OauthCardPathValue && OauthCardPathValue.GetValueKind() is JsonValueKind.String &&
            OauthCardPathValue.TryGetValue<string>(out string? OauthCardPathStrValue))
        {
    ...
    {
        ExtensionResponseSemanticsProperties properties = new();

        if (source.TryGetPropertyValue(nameof(ExtensionResponseSemanticsProperties.Title).ToFirstCharacterLowerCase(), out JsonNode? title) &&
            title is JsonValue titleValue && titleValue.GetValueKind() is JsonValueKind.String &&
            titleValue.TryGetValue<string>(out string? titleStrValue))
        {
        ...
        if (source.TryGetPropertyValue(nameof(ExtensionResponseSemanticsProperties.Subtitle).ToFirstCharacterLowerCase(), out JsonNode? subtitle) &&
            subtitle is JsonValue subtitleValue && subtitleValue.GetValueKind() is JsonValueKind.String &&
            subtitleValue.TryGetValue<string>(out string? subtitleStrValue))
        {
            properties.Subtitle = subtitleStrValue;
        }

        if (source.TryGetPropertyValue(nameof(ExtensionResponseSemanticsProperties.Url).ToFirstCharacterLowerCase(), out JsonNode? url) &&
            url is JsonValue urlValue && urlValue.GetValueKind() is JsonValueKind.String &&
            urlValue.TryGetValue<string>(out string? urlStrValue))
        {
            properties.Url = urlStrValue;
        }

        if (source.TryGetPropertyValue(nameof(ExtensionResponseSemanticsProperties.ThumbnailUrl).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? thumbnailUrl) &&
            thumbnailUrl is JsonValue thumbnailUrlValue && thumbnailUrlValue.GetValueKind() is JsonValueKind.String &&
            thumbnailUrlValue.TryGetValue<string>(out string? thumbnailUrlStrValue))
        {
            properties.ThumbnailUrl = thumbnailUrlStrValue;
        }

        if (source.TryGetPropertyValue(nameof(ExtensionResponseSemanticsProperties.InformationProtectionLabel).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? informationProtectionLabel) &&
            ...
            informationProtectionLabelValue.TryGetValue<string>(out string? informationProtectionLabelStrValue))
        {
            properties.InformationProtectionLabel = informationProtectionLabelStrValue;
        }

        if (source.TryGetPropertyValue(nameof(ExtensionResponseSemanticsProperties.TemplateSelector).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? templateSelector) &&
            templateSelector is JsonValue templateSelectorValue && templateSelectorValue.GetValueKind() is JsonValueKind.String &&
            templateSelectorValue.TryGetValue<string>(out string? templateSelectorStrValue))
        {
    ...
    {
        ExtensionSecurityInfo securityInfo = new();

        if (source.TryGetPropertyValue(nameof(ExtensionSecurityInfo.DataHandling).ToFirstCharacterLowerCase().ToSnakeCase(), out JsonNode? dataHandling) &&
            dataHandling is JsonArray dataHandlingArray)
        {
            List<string> dataHandlingList = [];

            foreach (JsonNode? item in dataHandlingArray)
            {
                if (item is JsonValue valueItem && valueItem.GetValueKind() is JsonValueKind.String &&
                    valueItem.TryGetValue<string>(out string? strValue))
                {
    ...
    private void WriteSecurityInfo(IOpenApiWriter writer, ExtensionSecurityInfo securityInfo)
    {
        ...
        if (securityInfo.DataHandling != null && securityInfo.DataHandling.Count > 0)
        {
            ...
            foreach (string item in securityInfo.DataHandling)
            {
