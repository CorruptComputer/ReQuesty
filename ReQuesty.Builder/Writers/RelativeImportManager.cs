...
public class RelativeImportManager
{
    ...
    public virtual (string, string, string) GetRelativeImportPathForUsing(CodeUsing codeUsing, CodeNamespace currentNamespace)
    {
        ...
        (string importSymbol, CodeElement? typeDef) = codeUsing.Declaration?.TypeDefinition is CodeElement td ? td switch
        {
    ...
    protected string GetImportRelativePathFromNamespaces(CodeNamespace currentNamespace, CodeNamespace importNamespace)
    {
        ...
        NamespaceDifferentialTracker result = currentNamespace.GetDifferential(importNamespace, prefix, separator);
    ...
    {
        string[] segments = remainingSegments.Select(x => x.ToFirstCharacterLowerCase()).ToArray();
