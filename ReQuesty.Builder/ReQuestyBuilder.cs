...
public partial class ReQuestyBuilder
{
    ...
    private async Task CleanOutputDirectoryAsync(CancellationToken cancellationToken)
    {
        if (config.CleanOutput && Directory.Exists(config.OutputPath))
        {
            ...
            foreach (string subDir in Directory.EnumerateDirectories(config.OutputPath))
            {
            ...
            foreach (string? subFile in Directory.EnumerateFiles(config.OutputPath)
    ...
    public async Task<(OpenApiUrlTreeNode?, OpenApiDiagnostic?)> GetUrlTreeNodeAsync(CancellationToken cancellationToken)
    {
        ...
        (int _, OpenApiUrlTreeNode? openApiTree, bool _, OpenApiDiagnostic? openApiDiagnostic) = await GetTreeNodeInternalAsync(inputPath, false, sw, cancellationToken).ConfigureAwait(false);
...
#pragma warning restore CA2007
        ...
        // Parse OpenAPI
        sw.Start();
        ReadResult? readResult = await CreateOpenApiDocumentWithResultAsync(input, generating, cancellationToken).ConfigureAwait(false);
        ...
        StopLogAndReset(sw, $"step {++stepId} - updating generation configuration from requesty extension - took");
        ...
        if (openApiDocument != null)
        {
            ...
            if (!config.NoWorkspace)
            {
                string hashCode = await openApiDocument.GetHashCodeAsync(cancellationToken).ConfigureAwait(false);
    ...
    private LanguagesInformation? GetLanguagesInformationInternal()
    {
        ...
        if (openApiDocument.Extensions.TryGetValue(OpenApiReQuestyExtension.Name, out IOpenApiExtension? ext) && ext is OpenApiReQuestyExtension requestyExt)
        {
            return requestyExt.LanguagesInformation;
        }
    ...
    public async Task<bool> GenerateClientAsync(CancellationToken cancellationToken)
    {
        return await GenerateConsumerAsync(async (sw, stepId, openApiTree, CancellationToken) =>
        {
            // Create Source Model
            sw.Start();
            CodeNamespace generatedCode = CreateSourceModel(openApiTree);
            ...
            if (config.ExportPublicApi)
            {
                ...
                sw.Start();
                FileStream fileStream = File.Create(Path.Combine(config.OutputPath, PublicApiExportService.DomExportFileName));
    ...
    private async Task<bool> GenerateConsumerAsync(Func<Stopwatch, int, OpenApiUrlTreeNode?, CancellationToken, Task<int>> innerGenerationSteps, CancellationToken cancellationToken)
    {
        ...
        try
        {
            (int stepId, OpenApiUrlTreeNode? openApiTree, bool shouldGenerate, OpenApiDiagnostic? _) = await GetTreeNodeInternalAsync(inputPath, true, sw, cancellationToken).ConfigureAwait(false);
    ...
    private static Dictionary<Glob, HashSet<NetHttpMethod>> GetFilterPatternsFromConfiguration(HashSet<string> configPatterns)
    {
        ...
        {
            string[] splat = x.Split('#', StringSplitOptions.RemoveEmptyEntries);
            Glob glob = Glob.Parse(ReplaceAllIndexesWithWildcard(splat[0]));
            IEnumerable<NetHttpMethod?> operationTypes = splat.Length > 1 ?
    ...
    internal void FilterPathsByPatterns(OpenApiDocument doc)
    {
        ...
        List<Glob> nonOperationIncludePatterns = includePatterns.Where(static x => x.Value.Count == 0).Select(static x => x.Key).ToList();
        List<Glob> nonOperationExcludePatterns = excludePatterns.Where(static x => x.Value.Count == 0).Select(static x => x.Key).ToList();
        List<KeyValuePair<Glob, HashSet<NetHttpMethod>>> operationIncludePatterns = includePatterns.Where(static x => x.Value.Count != 0).ToList();
        ...
        List<KeyValuePair<Glob, HashSet<NetHttpMethod>>> operationExcludePatterns = excludePatterns.Where(static x => x.Value.Count != 0).ToList();

        if (operationIncludePatterns.Count != 0 || operationExcludePatterns.Count != 0)
        {
            foreach (KeyValuePair<string, IOpenApiPathItem> path in doc.Paths.Where(x => !nonOperationIncludePatterns.Any(y => y.IsMatch(x.Key))))
            {
            ...
            foreach (KeyValuePair<string, IOpenApiPathItem> path in doc.Paths)
            {
                string pathString = path.Key;
            ...
            foreach (KeyValuePair<string, IOpenApiPathItem> path in doc.Paths.Where(static x => x.Value.Operations is null || x.Value.Operations.Count == 0).ToList())
            {
    ...
    public static string GetDeeperMostCommonNamespaceNameForModels(OpenApiDocument document)
    {
        ...
        string[] distinctKeys = document.Components
                                .Schemas
                                .Keys
                                ...
                                .ToArray();
        if (distinctKeys.FirstOrDefault() is not string longestKey)
        {
            return string.Empty;
        }

        string candidate = string.Empty;
        string[] longestKeySegments = longestKey.Split(NsNameSeparator, StringSplitOptions.RemoveEmptyEntries);
        foreach (string segment in longestKeySegments)
        {
            string testValue = (candidate + NsNameSeparator + segment).Trim(NsNameSeparator);
    ...
    public OpenApiUrlTreeNode CreateUriSpace(OpenApiDocument doc)
    {
        ...
        stopwatch.Start();
        OpenApiUrlTreeNode node = OpenApiUrlTreeNode.Create(doc, Constants.DefaultOpenApiLabel);
    ...
    public CodeNamespace CreateSourceModel(OpenApiUrlTreeNode? root)
    {
        ...
        CodeNamespace codeNamespace = rootNamespace.AddNamespace(config.ClientNamespaceName);
    ...
    private void CreateRequestBuilderClass(CodeNamespace currentNamespace, OpenApiUrlTreeNode currentNode, OpenApiUrlTreeNode rootNode)
    {
        ...
        CodeClass codeClass;
        bool isApiClientClass = currentNode == rootNode;
        if (isApiClientClass)
        {
        ...
        {
            CodeNamespace targetNS = currentNode.DoesNodeBelongToItemSubnamespace() ? currentNamespace.EnsureItemNamespace() : currentNamespace;
            string className = currentNode.DoesNodeBelongToItemSubnamespace() ? currentNode.GetNavigationPropertyName(config.StructuredMimeTypes, ItemRequestBuilderSuffix, placeholder: TrailingSlashPlaceholder) : currentNode.GetNavigationPropertyName(config.StructuredMimeTypes, RequestBuilderSuffix, placeholder: TrailingSlashPlaceholder);
        ...
        foreach (OpenApiUrlTreeNode? child in currentNode.Children.Select(static x => x.Value))
        {
            string propIdentifier = child.GetNavigationPropertyName(config.StructuredMimeTypes, placeholder: TrailingSlashPlaceholder);
            string propType = child.GetNavigationPropertyName(config.StructuredMimeTypes, child.DoesNodeBelongToItemSubnamespace() ? ItemRequestBuilderSuffix : RequestBuilderSuffix, placeholder: TrailingSlashPlaceholder);
            ...
            {
                CodeParameter indexerParameterType = GetIndexerParameter(child);
            ...
            {
                string description = child.GetPathItemDescription(Constants.DefaultOpenApiLabel).CleanupDescription();
                CodeProperty? prop = CreateProperty(propIdentifier, propType, kind: CodePropertyKind.RequestBuilder); // we should add the type definition here but we can't as it might not have been generated yet
                if (prop is null)
                {
        ...
        if (currentNode.HasOperations(Constants.DefaultOpenApiLabel))
        {
            ...
            if (currentNode.PathItems.TryGetValue(Constants.DefaultOpenApiLabel, out IOpenApiPathItem? pathItem) && pathItem.Operations is not null)
            {
                foreach (KeyValuePair<NetHttpMethod, OpenApiOperation> operation in pathItem.Operations)
                {
        ...
        if (rootNamespace != null)
        {
            Parallel.ForEach(currentNode.Children.Values, parallelOptions, childNode =>
            {
                ...
                {
                    CodeNamespace targetNamespace = rootNamespace.FindOrAddNamespace(targetNamespaceName);
    ...
    {
        foreach (IOpenApiParameter parameter in currentNode.GetPathParametersForCurrentSegment())
        {
            ...
            CodeType parameterType = GetPrimitiveType(parameter.Schema ?? parameter.Content?.Values.FirstOrDefault()?.Schema) ??
            new CodeType
            {
                Name = "string",
                IsExternal = true,
    ...
    private void CreateUrlManagement(CodeClass currentClass, OpenApiUrlTreeNode currentNode, bool isApiClientClass)
    {
        ...
        CodeProperty requestAdapterProperty = new()
        {
        ...
        CodeProperty pathParametersProperty = new()
        {
        ...
        {
            string factoryInterfaceName = $"{BackingStoreInterface}Factory";
            CodeParameter backingStoreParam = new()
            {
                Name = "backingStore",
                Optional = true,
                Documentation = new()
        ...
        if (!isApiClientClass)
        {
            CodeMethod overloadCtor = (CodeMethod)constructor.Clone();
    ...
    {
        if (serializers.Count == 1 && serializers.Contains("none"))
        {
            return [];
        }
    ...
    {
        CodeType[] unmappedTypes = GetUnmappedTypeDefinitions(codeElement).Distinct().ToArray();

        List<CodeType> unmappedTypesWithNoName = unmappedTypes.Where(static x => string.IsNullOrEmpty(x.Name)).ToList();
        ...
        IEnumerable<CodeType> unmappedTypesWithName = unmappedTypes.Except(unmappedTypesWithNoName);

        List<CodeType> unmappedRequestBuilderTypes = unmappedTypesWithName
                                .Where(static x =>
        ...
        {
            CodeNamespace? parentNS = x.Parent?.Parent?.Parent as CodeNamespace;
    ...
    private IEnumerable<CodeType> GetUnmappedTypeDefinitions(CodeElement codeElement)
    {
        ...
        {
            CodeMethod method => filterUnmappedTypeDefinitions(method.Parameters.Select(static x => x.Type).Union([method.ReturnType])).Union(childElementsUnmappedTypes),
            CodeProperty property => filterUnmappedTypeDefinitions([property.Type]).Union(childElementsUnmappedTypes),
            CodeIndexer indexer => filterUnmappedTypeDefinitions([indexer.ReturnType]).Union(childElementsUnmappedTypes),
    ...
    {
        string parameterName = (currentNode.AdditionalData.TryGetValue(Constants.ReQuestySegmentNameTreeNodeExtensionKey, out List<string>? newNames) && newNames is { Count: > 0 } ?
                        newNames[0] :
                        currentNode.Segment).Trim('{', '}');
        IDictionary<string, IOpenApiPathItem> pathItems = GetPathItems(currentNode);
        IOpenApiParameter? parameter = pathItems.TryGetValue(Constants.DefaultOpenApiLabel, out IOpenApiPathItem? pathItem) ?
                        ...
                        default;
        CodeType type = parameter switch
        {
            null => DefaultIndexerParameterType,
            _ => GetPrimitiveType(parameter.Schema),
        } ?? DefaultIndexerParameterType;
        type.IsNullable = false;
        string segment = currentNode.DeduplicatedSegment();
        CodeParameter result = new()
        {
            Type = type,
    ...
    {
        string propertyName = childIdentifier.CleanupSymbolName();
        if (structuralPropertiesReservedNameProvider.ReservedNames.Contains(propertyName))
        {
            propertyName += "Property";
        }

        CodeTypeBase? resultType = existingType ?? GetPrimitiveType(propertySchema, childType);
        ...
        CodeProperty prop = new()
        {
            ...
            IsPrimaryErrorMessage = kind == CodePropertyKind.Custom &&
                                        ...
                                        propertySchema.Extensions.TryGetValue(OpenApiPrimaryErrorMessageExtension.Name, out IOpenApiExtension? openApiExtension) &&
        ...
        if (kind == CodePropertyKind.Custom &&
            ...
            stringDefaultJsonValue.TryGetValue<string>(out string? stringDefaultValue) &&
    ...
    private static CodeType? GetPrimitiveType(IOpenApiSchema? typeSchema, string? childType = default)
    {
        ...
        List<JsonSchemaType?> typeNames = [typeSchema?.Items?.Type, typeSchema?.Type];
    ...
    private void AddErrorMappingsForExecutorMethod(OpenApiUrlTreeNode currentNode, OpenApiOperation operation, CodeMethod executorMethod)
    {
        ...
        foreach (KeyValuePair<string, IOpenApiResponse> response in operation.Responses.Where(x => errorStatusCodes.Contains(x.Key)))
        {
        ...
        if (operation.Responses.TryGetValue(DefaultResponseIndicator, out IOpenApiResponse? defaultResponse) && defaultResponse.GetResponseSchema(config.StructuredMimeTypes) is { } errorSchema)
        {
    ...
    private void AddErrorMappingToExecutorMethod(OpenApiUrlTreeNode currentNode, OpenApiOperation operation, CodeMethod executorMethod, IOpenApiSchema errorSchema, IOpenApiResponse response, string errorCode)
    {
        if (modelsNamespace != null)
        {
            CodeElement parentElement = response is not OpenApiResponseReference && errorSchema is not OpenApiSchemaReference
                ...
                : modelsNamespace;
            CodeTypeBase errorType = CreateModelDeclarations(currentNode, errorSchema, operation, parentElement, $"{errorCode}Error", response: response);
    ...
    private void CreateOperationMethods(OpenApiUrlTreeNode currentNode, NetHttpMethod operationType, OpenApiOperation operation, CodeClass parentClass)
    {
        try
        {
            CodeClass? parameterClass = CreateOperationParameterClass(currentNode, operationType, operation, parentClass);
            CodeClass requestConfigClass = parentClass.AddInnerClass(new CodeClass
            {
            ...
            IOpenApiSchema? schema = operation.GetResponseSchema(config.StructuredMimeTypes);
            DomHttpMethod method = Enum.Parse<DomHttpMethod>(operationType.ToString(), true);
            DeprecationInformation deprecationInformation = operation.GetDeprecationInformation();
            (CodeTypeBase?, CodeTypeBase?) returnTypes = GetExecutorMethodReturnType(currentNode, schema, operation, parentClass, operationType);
            CodeMethod executorMethod = new()
            {
            ...
            if (operation.Extensions is not null && operation.Extensions.TryGetValue(OpenApiPagingExtension.Name, out IOpenApiExtension? extension) && extension is OpenApiPagingExtension pagingExtension)
            {
            ...
            string operationUrlTemplate = currentNode.GetUrlTemplate(operationType);
            ...
            IEnumerable<string> mediaTypes = (schema, operation.Responses is null) switch
            {
                (_, true) => [],
    ...
    private void AddRequestBuilderMethodParameters(OpenApiUrlTreeNode currentNode, NetHttpMethod operationType, OpenApiOperation operation, CodeClass requestConfigClass, CodeMethod method)
    {
        if (operation.RequestBody is not null)
        {
            if (operation.GetRequestSchema(config.StructuredMimeTypes) is IOpenApiSchema requestBodySchema)
            {
                    ...
                    && requestBodySchema.Properties is not null)
                {
                    ...
                    if (mediaType.Encoding is not null && mediaType.Encoding.Count != 0)
                    {
                        ...
                        foreach (KeyValuePair<string, OpenApiEncoding> encodingEntry in mediaType.Encoding
                    ...
                    {
                        requestBodyType = new CodeType { Name = "MultipartBody", IsExternal = true, };
                        foreach (IOpenApiSchema? property in requestBodySchema.Properties.Values.Where(schema => IsSupportedMultipartDefault(schema, config.StructuredMimeTypes)))
                        {
            ...
            else if (operation.RequestBody.Content is { Count: > 0 })
            {
                ...
                string[] contentTypes = operation.RequestBody.Content.Select(static x => x.Key).ToArray();
    ...
    private string GetModelsNamespaceNameFromReferenceId(string? referenceId)
    {
        ...
        referenceId = referenceId.Trim(NsNameSeparator);
        int lastDotIndex = referenceId.LastIndexOf(NsNameSeparator);
        string namespaceSuffix = lastDotIndex != -1 ? $".{referenceId[..lastDotIndex]}" : string.Empty;
    ...
    {
        string className = string.IsNullOrEmpty(typeNameForInlineSchema) ? currentNode.GetClassName(config.StructuredMimeTypes, operation: operation, suffix: classNameSuffix, response: response, schema: schema, requestBody: isRequestBody).CleanupSymbolName() : typeNameForInlineSchema;
        CodeElement codeDeclaration = AddModelDeclarationIfDoesntExist(currentNode, operation, schema, className, codeNamespace);
        return new CodeType
        {
    ...
    {
        IOpenApiSchema[] flattenedAllOfs = schema.AllOf.FlattenSchemaIfRequired(static x => x.AllOf).ToArray();
        string? referenceId = schema.GetReferenceId();
        string shortestNamespaceName = GetModelsNamespaceNameFromReferenceId(referenceId);
        CodeNamespace codeNamespaceFromParent = GetShortestNamespace(codeNamespace, schema);
        if (rootNamespace is null)
        {
            throw new InvalidOperationException("Root namespace is not set");
        }

        CodeNamespace shortestNamespace = string.IsNullOrEmpty(referenceId) ? codeNamespaceFromParent : rootNamespace.FindOrAddNamespace(shortestNamespaceName);
        IOpenApiSchema[] inlineSchemas = Array.FindAll(flattenedAllOfs, static x => !x.IsReferencedSchema());
        IOpenApiSchema[] referencedSchemas = Array.FindAll(flattenedAllOfs, static x => x.IsReferencedSchema());
        bool rootSchemaHasProperties = schema.HasAnyProperty();
        ...
        bool shouldNameLookupConsiderSubSchemas = schema.IsSemanticallyMeaningful() || string.IsNullOrEmpty(referenceId);
        string className = (schema.GetSchemaName(shouldNameLookupConsiderSubSchemas) is string cName && !string.IsNullOrEmpty(cName) ?
                cName :
            ...
            .CleanupSymbolName();
        CodeElement codeDeclaration = (rootSchemaHasProperties, inlineSchemas, referencedSchemas, isViaDiscriminator) switch
        {
        ...
        if (codeDeclaration is not CodeClass currentClass)
        {
            throw new InvalidOperationException("Inheritance is only supported for classes");
        }
    ...
    {
        string typeName = string.IsNullOrEmpty(typeNameForInlineSchema) ? currentNode.GetClassName(config.StructuredMimeTypes, operation: operation, suffix: suffixForInlineSchema, schema: schema, requestBody: isRequestBody).CleanupSymbolName() : typeNameForInlineSchema;
        int typesCount = schema.AnyOf?.Count ?? schema.OneOf?.Count ?? 0;
        ...
        {
            string className = targetSchema.GetSchemaName().CleanupSymbolName();
            CodeNamespace shortestNamespace = GetShortestNamespace(codeNamespace, targetSchema);
            return new CodeType
            {
        ...
        (CodeComposedTypeBase unionType, IList<IOpenApiSchema>? schemas) = (schema.IsExclusiveUnion(), schema.IsInclusiveUnion()) switch
        {
        ...
        int membersWithNoName = 0;
        if (schemas is not null)
        {
            foreach (IOpenApiSchema currentSchema in schemas)
            {
                CodeNamespace shortestNamespace = GetShortestNamespace(codeNamespace, currentSchema);
                string className = currentSchema.GetSchemaName().CleanupSymbolName();
    ...
    {
        if (!schema.Type.HasValue || (schema.Type.Value & typeToScan) != typeToScan)
        {
            return;
        }

        OpenApiSchema temporarySchema = new() { Type = typeToScan, Format = schema.Format };
    ...
    {
        (CodeNamespace codeNamespace, IOpenApiResponse? responseValue, string suffix) = schema.IsReferencedSchema() switch
        {
    ...
    {
        CodeNamespace localNameSpace = GetSearchNamespace(currentNode, currentNamespace);
    ...
    private static CodeEnum? AddEnumDeclaration(OpenApiUrlTreeNode currentNode, IOpenApiSchema schema, string declarationName, CodeNamespace currentNamespace)
    {
        if (schema.IsEnum())
        {
            string schemaDescription = schema.Description.CleanupDescription();
            OpenApiEnumFlagsExtension? enumFlagsExtension = null;
            if (schema.Extensions is not null && schema.Extensions.TryGetValue(OpenApiEnumFlagsExtension.Name, out IOpenApiExtension? rawExtension) &&
                rawExtension is OpenApiEnumFlagsExtension flagsExtension)
            {
                enumFlagsExtension = flagsExtension;
            }
            CodeEnum newEnum = new()
            {
    ...
    private static void SetEnumOptions(IOpenApiSchema schema, CodeEnum target)
    {
        ...
        if (schema.Extensions is not null && schema.Extensions.TryGetValue(OpenApiEnumValuesDescriptionExtension.Name, out IOpenApiExtension? rawExtension) && rawExtension is OpenApiEnumValuesDescriptionExtension localExtInfo)
        {
        ...
        target.AddOption(schema.Enum?.OfType<JsonValue>()
                        ...
                        .Select((x) =>
                        {
                            EnumDescription? optionDescription = extensionInformation?.ValuesDescriptions.Find(y => y.Value.Equals(x, StringComparison.OrdinalIgnoreCase));
    ...
    private CodeClass AddModelClass(OpenApiUrlTreeNode currentNode, IOpenApiSchema schema, string declarationName, CodeNamespace currentNamespace, OpenApiOperation? currentOperation, CodeClass? inheritsFrom = null)
    {
        ...
        bool includeAdditionalDataProperties = config.IncludeAdditionalData && (schema.AdditionalPropertiesAllowed || schema.AdditionalProperties is not null);
        AddSerializationMembers(newClassStub, includeAdditionalDataProperties, config.UsesBackingStore, static s => s);

        CodeClass newClass = currentNamespace.AddClass(newClassStub).First();
        ModelClassBuildLifecycle lifecycle = classLifecycles.GetOrAdd(currentNamespace.Name + "." + declarationName, static n => new());
        ...
        IOpenApiSchema lookupSchema = schema.GetMergedSchemaOriginalReferenceId() is string originalName ?
            new OpenApiSchemaReference(originalName, openApiDocument) :
            schema;
        // Recurse into the discriminator & generate its referenced types
        IEnumerable<KeyValuePair<string, CodeType>> mappings = GetDiscriminatorMappings(currentNode, lookupSchema, currentNamespace, newClass, currentOperation)
    ...
    private IEnumerable<KeyValuePair<string, CodeType>> GetDiscriminatorMappings(OpenApiUrlTreeNode currentNode, IOpenApiSchema schema, CodeNamespace currentNamespace, CodeClass? baseClass, OpenApiOperation? currentOperation)
    {
        ...
        return schema.GetDiscriminatorMappings(inheritanceIndex)
                .Union(baseClass is not null && modelsNamespace is not null &&
                        (openApiDocument?.Components?.Schemas?.TryGetValue(baseClass.GetComponentSchemaName(modelsNamespace), out IOpenApiSchema? componentSchema) ?? false) ?
    ...
    {
        CodeClass[] classes = currentNamespace.Classes.ToArray();
    ...
    {
        if (modelsNamespace is null || rootNamespace is null || modelsNamespace.Parent is not CodeNamespace clientNamespace)
        {
            return;
        }

        ITypeDefinition[] reusableModels = GetAllModels(modelsNamespace).ToArray();//to avoid multiple enumerations
        CodeElement[] modelsDirectlyInUse = GetTypeDefinitionsInNamespace(rootNamespace).Union(multipartPropertiesModels.Keys).ToArray();
        HashSet<CodeClass> classesDirectlyInUse = modelsDirectlyInUse.OfType<CodeClass>().ToHashSet();
        ConcurrentDictionary<CodeClass, ConcurrentBag<CodeClass>> allModelClassesIndex = GetDerivationIndex(GetAllModels(clientNamespace).OfType<CodeClass>());
        IEnumerable<CodeClass> derivedClassesInUse = GetDerivedDefinitions(allModelClassesIndex, classesDirectlyInUse.ToArray());
        IEnumerable<CodeClass> baseOfModelsInUse = classesDirectlyInUse.SelectMany(static x => x.GetInheritanceTree(false, false));
        HashSet<CodeClass> classesInUse = derivedClassesInUse.Union(classesDirectlyInUse).Union(baseOfModelsInUse).ToHashSet();
        ConcurrentDictionary<CodeClass, ConcurrentBag<CodeClass>> reusableClassesDerivationIndex = GetDerivationIndex(reusableModels.OfType<CodeClass>());
        ConcurrentDictionary<CodeClass, ConcurrentBag<CodeClass>> reusableClassesInheritanceIndex = GetInheritanceIndex(allModelClassesIndex);
        HashSet<ITypeDefinition> relatedModels = classesInUse.SelectMany(x => GetRelatedDefinitions(x, reusableClassesDerivationIndex, reusableClassesInheritanceIndex)).Union(modelsDirectlyInUse.OfType<CodeEnum>()).ToHashSet();// re-including models directly in use for enums
        Parallel.ForEach(reusableModels, parallelOptions, x =>
        {
            if (relatedModels.Contains(x) || classesInUse.Contains(x))
            {
                return;
            }
            ...
            {
                System.Collections.ObjectModel.Collection<CodeClass> parents = currentClass.GetInheritanceTree(false, false);
                if (parents.Any(y => classesDirectlyInUse.Contains(y)))
                {
                    return; // to support the inheritance recursive downcast
                }

                foreach (CodeClass baseClass in parents) // discriminator might also be in grand parent types
                {
        ...
        foreach (CodeNamespace leafNamespace in FindLeafNamespaces(modelsNamespace))
        {
    ...
    {
        ConcurrentDictionary<CodeClass, ConcurrentBag<CodeClass>> result = new();
    ...
    private ConcurrentDictionary<CodeClass, ConcurrentBag<CodeClass>> GetInheritanceIndex(ConcurrentDictionary<CodeClass, ConcurrentBag<CodeClass>> derivedIndex)
    {
        ...
        {
            foreach (CodeClass? derivedClass in entry.Value)
            {
    ...
    {
        if (!currentNamespace.Namespaces.Any())
        {
            return [currentNamespace];
        }
    ...
    private static void RemoveEmptyNamespaces(CodeNamespace currentNamespace, CodeNamespace stopAtNamespace)
    {
        ...
        if (currentNamespace.Parent is not CodeNamespace parentNamespace)
        {
            return;
        }
    ...
    {
        CodeClass[] currentDerived = modelsInUse.SelectMany(x => models.TryGetValue(x, out ConcurrentBag<CodeClass>? res) ? res : Enumerable.Empty<CodeClass>()).ToArray();
    ...
    {
        visited ??= new();
        if (currentElement is not CodeClass currentClass || !visited.TryAdd(currentClass, true))
        {
            return [];
        }

        IEnumerable<ITypeDefinition> propertiesDefinitions = currentClass.Properties
                            ...
                            .SelectMany(x => x is CodeClass classDefinition ?
                                            (inheritanceIndex.TryGetValue(classDefinition, out ConcurrentBag<CodeClass>? res) ? res : Enumerable.Empty<CodeClass>())
        ...
        IEnumerable<ITypeDefinition> propertiesParentTypes = propertiesDefinitions.OfType<CodeClass>().SelectMany(static x => x.GetInheritanceTree(false, false)).OfType<ITypeDefinition>();
        IEnumerable<ITypeDefinition> propertiesParentRelatedTypes = propertiesParentTypes.SelectMany(x => GetRelatedDefinitions(x, derivedIndex, inheritanceIndex, visited));
        IEnumerable<ITypeDefinition> propertiesDefinitionsRelatedTypes = propertiesDefinitions.SelectMany(x => GetRelatedDefinitions(x, derivedIndex, inheritanceIndex, visited));

        return propertiesDefinitions
                .Union(propertiesParentTypes)
                .Union(propertiesParentRelatedTypes)
                .Union(propertiesDefinitionsRelatedTypes);
    }
    ...
    {
        IEnumerable<CodeMethod> requestExecutors = GetAllNamespaces(currentNamespace)
    ...
    private CodeType? GetCodeTypeForMapping(OpenApiUrlTreeNode currentNode, string referenceId, CodeNamespace currentNamespace, CodeClass? baseClass, OpenApiOperation? currentOperation)
    {
        ...
        IOpenApiSchema schemaClone = discriminatorSchema.CreateShallowCopy();
    ...
    internal static void AddSerializationMembers(CodeClass model, bool includeAdditionalProperties, bool usesBackingStore, Func<string, string> refineMethodName)
    {
        ...
        {
            CodeMethod deserializeProp = new()
            {
        ...
        {
            CodeMethod serializeMethod = new()
            {
            ...
            !(model.GetGreatestGrandparent(model)?.ContainsMember(AdditionalDataPropName) ?? false))
        {
            ...
            CodeProperty additionalDataProp = new()
            {
        ...
        {
            CodeProperty backingStoreProperty = new()
            {
    ...
    {
        IOpenApiParameter[] parameters = (node.PathItems[Constants.DefaultOpenApiLabel].Parameters ?? Enumerable.Empty<IOpenApiParameter>()).Union(operation.Parameters ?? Enumerable.Empty<IOpenApiParameter>()).Where(static p => p.In == ParameterLocation.Query).ToArray();
        if (parameters.Length != 0)
        {
            CodeClass parameterClass = parentClass.AddInnerClass(new CodeClass
            {
    ...
    private void AddPropertyForQueryParameter(OpenApiUrlTreeNode node, NetHttpMethod operationType, IOpenApiParameter parameter, CodeClass parameterClass)
    {
        ...
        {
            IOpenApiSchema enumSchema = parameter.Schema.IsArray() ? parameter.Schema.Items! : parameter.Schema;
            CodeNamespace codeNamespace = enumSchema.IsReferencedSchema() switch
            {
            ...
            CodeNamespace shortestNamespace = GetShortestNamespace(codeNamespace, enumSchema);
            string enumName = enumSchema.GetSchemaName().CleanupSymbolName();
    ...
    {
        foreach (ModelClassBuildLifecycle lifecycle in classLifecycles.Values)
        {
            lifecycle.Dispose();
        }
