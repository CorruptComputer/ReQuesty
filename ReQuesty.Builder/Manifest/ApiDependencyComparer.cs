...
public class ApiDependencyComparer : IEqualityComparer<ApiDependency>
{
    ...
    {
        if (dependency.Extensions?.TryGetValue(GenerationConfiguration.ReQuestyHashManifestExtensionKey, out JsonNode? n0) ==
    ...
    public bool Equals(ApiDependency? x, ApiDependency? y)
    {
        ...
        if (!_stringComparer.Equals(x.ApiDescriptionUrl, y.ApiDescriptionUrl))
        {
            return false;
        }

        if (!_stringComparer.Equals(x.ApiDescriptionVersion, y.ApiDescriptionVersion))
        {
            return false;
        }
        ...
        bool requestsEqual = !_compareRequests || GetOrderedRequests(x.Requests).SequenceEqual(GetOrderedRequests(y.Requests), _requestInfoComparer);
    ...
    public int GetHashCode([DisallowNull] ApiDependency obj)
    {
        ...
        foreach (RequestInfo request in GetOrderedRequests(obj.Requests))
        {
