...
{
    protected static readonly char[] UnderscoreArray = ['_'];
    ...
    {
        serializationWriterFactoryInterfaceAndRegistrationFullName ??= [];
        ...
        {
            List<string> cumulatedSymbols = currentMethod.DeserializerModules
    ...
    private static bool ReplaceSerializationModules(CodeElement generatedCode, Func<CodeMethod, HashSet<string>> propertyGetter, Action<CodeMethod, HashSet<string>> propertySetter, HashSet<string> initialNames, HashSet<string> moduleNames)
    {
        if (generatedCode is CodeMethod currentMethod &&
            currentMethod.IsOfKind(CodeMethodKind.ClientConstructor))
        {
            HashSet<string> modules = propertyGetter.Invoke(currentMethod);
    ...
    protected static void ReplacePropertyNames(CodeElement current, HashSet<CodePropertyKind> propertyKindsToReplace, Func<string, string> refineAccessorName)
    {
        ...
        if (propertyKindsToReplace is null || propertyKindsToReplace.Count == 0)
        {
            return;
        }
        ...
        {
            string refinedName = refineAccessorName(currentProperty.Name);
    ...
    {
        ArgumentNullException.ThrowIfNull(refineAccessorName);
        bool isSetterPrefixEmpty = string.IsNullOrEmpty(setterPrefix);
        bool isGetterPrefixEmpty = string.IsNullOrEmpty(getterPrefix);
        if (propertyKindsToAddAccessors is null || propertyKindsToAddAccessors.Count == 0)
        {
            return;
        }
            ...
            !parentClass.IsOfKind(CodeClassKind.QueryParameters))
        {
            ...
            string accessorName = refineAccessorName(current, currentProperty.Name.ToFirstCharacterUpperCase());
    ...
    protected static void ReplaceReservedModelTypes(CodeElement current, IReservedNamesProvider provider, Func<string, string> replacement) =>
        ReplaceReservedNames(current,
            provider,
            replacement,
            codeElementExceptions: [typeof(CodeNamespace)],
    ...
    {
        Func<string, string> newReplacement = replacement;
        int index = 0;
        while (true)
        {
            if (index > 0)
    ...
    protected static void ReplaceReservedNames(CodeElement current, IReservedNamesProvider provider, Func<string, string> replacement, HashSet<Type>? codeElementExceptions = null, Func<CodeElement, bool>? shouldReplaceCallback = null)
    {
        ...
        bool shouldReplace = shouldReplaceCallback?.Invoke(current) ?? true;
        bool isNotInExceptions = !codeElementExceptions?.Contains(current.GetType()) ?? true;
        if (current is CodeClass currentClass &&
            isNotInExceptions &&
            shouldReplace &&
            currentClass.StartBlock is ClassDeclaration currentDeclaration)
            ...
            (shouldReplaceCallback?.Invoke(current) ?? true))// re-invoke the callback if present as conditions above may have renamed dependencies.
        {
            ...
            string replacementName = replacement.Invoke(current.Name);
    ...
    {
        string[] segments = currentNamespace.Name.Split('.');
    ...
    {
        ArgumentNullException.ThrowIfNull(current);
        CodeUsing[] usingsToAdd = evaluators.Where(x => x.CodeElementEvaluator.Invoke(current))
                        .SelectMany(usingSelector)
                        .ToArray();
        if (usingsToAdd.Length != 0)
        {
            IBlock parentBlock = current.GetImmediateParentOfType<IBlock>();
            IBlock targetBlock = parentBlock.Parent is CodeClass parentClassParent ? parentClassParent : parentBlock;
    ...
    protected static void ConvertUnionTypesToWrapper(CodeElement currentElement, bool usesBackingStore, Func<string, string> refineMethodName, bool supportInnerClasses = true, string markerInterfaceNamespace = "", string markerInterfaceName = "", string markerMethodName = "")
    {
        ...
        if (currentElement.Parent is CodeClass parentClass)
        {
            if (currentElement is CodeMethod currentMethod)
            {
                ...
                {
                    foreach (CodeParameter? currentParameter in currentMethod.Parameters.Where(static x => x.Type is CodeComposedTypeBase))
                    {
                ...
                {
                    foreach (CodeComposedTypeBase errorUnionType in currentMethod.ErrorMappings.Select(static x => x.Value).OfType<CodeComposedTypeBase>())
                    {
    ...
    private static CodeType ConvertComposedTypeToWrapper(CodeClass codeClass, CodeComposedTypeBase codeComposedType, bool usesBackingStore, Func<string, string> refineMethodName, bool supportsInnerClasses, string markerInterfaceNamespace, string markerInterfaceName, string markerMethodName)
    {
        ...
        {
            CodeNamespace @namespace = codeClass.GetImmediateParentOfType<CodeNamespace>();
        ...
        ReQuestyBuilder.AddDiscriminatorMethod(newClass, codeComposedType.DiscriminatorInformation.DiscriminatorPropertyName, codeComposedType.DiscriminatorInformation.DiscriminatorMappings, refineMethodName);
        return new CodeType
        {
            Name = newClass.Name,
    ...
    protected static void MoveClassesWithNamespaceNamesUnderNamespace(CodeElement currentElement)
    {
        ...
        {
            CodeNamespace? childNamespaceWithClassName = parentNamespace.GetChildElements(true)
    ...
    internal void DisableActionOf(CodeElement current, params CodeParameterKind[] kinds)
    {
        ...
        {
            foreach (CodeParameter? parameter in currentMethod.Parameters.Where(x => x.Type.ActionOf && x.IsOfKind(kinds)))
            {
    ...
    internal void AddInnerClasses(CodeElement current, bool prefixClassNameWithParentName, string queryParametersBaseClassName = "", bool addToParentNamespace = false, Func<string, string, string>? nameFactory = default)
    {
        ...
        {
            CodeNamespace parentNamespace = currentClass.GetImmediateParentOfType<CodeNamespace>();
            IEnumerable<CodeClass> innerClasses = currentClass
                                    .Methods
            ...
            IEnumerable<CodeClass> nestedQueryParameters = innerClasses
            ...
            List<CodeClass> nestedClasses = [.. innerClasses, .. nestedQueryParameters];

            foreach (CodeClass nestedClass in nestedClasses)
            {
                if (nestedClass.Parent is not CodeClass parentClass)
                {
                    continue;
                }
...
#pragma warning restore CA1051 // Do not declare visible instance fields
    ...
    protected static void AddPropertiesAndMethodTypesImports(CodeElement current, bool includeParentNamespaces, bool includeCurrentNamespace, bool compareOnDeclaration, Func<IEnumerable<CodeTypeBase>, IEnumerable<CodeTypeBase>>? codeTypeFilter = default)
    {
        ...
        {
            IEnumerable<CodeElement> currentClassChildren = currentClass.GetChildElements(true);
            IEnumerable<CodeType> inheritTypes = currentClassDeclaration.Inherits?.AllTypes ?? [];
            IEnumerable<CodeTypeBase> propertiesTypes = currentClass
                                .Properties
                                .Where(static x => !x.ExistsInBaseType)
                                .Select(static x => x.Type)
                                .Distinct();
            IEnumerable<CodeMethod> methods = currentClass.Methods;
            IEnumerable<CodeTypeBase> methodsReturnTypes = methods
                                .Select(static x => x.ReturnType)
                                .Distinct();
            IEnumerable<CodeTypeBase> methodsParametersTypes = methods
                                .SelectMany(static x => x.Parameters)
                                ...
                                .Select(static x => x.Type)
                                .Distinct();
            IEnumerable<CodeTypeBase> indexerTypes = currentClassChildren
                                .OfType<CodeIndexer>()
                                .Select(static x => x.ReturnType)
                                .Distinct();
            IEnumerable<CodeTypeBase> errorTypes = currentClassChildren
                                .OfType<CodeMethod>()
                                .Where(static x => x.IsOfKind(CodeMethodKind.RequestExecutor))
                                .SelectMany(static x => x.ErrorMappings)
                                .Select(static x => x.Value)
                                .Distinct();
            IEnumerable<CodeTypeBase> typesCollection = propertiesTypes
    ...
    protected static void CrawlTree(CodeElement currentElement, Action<CodeElement> function, bool innerOnly = true)
    {
        ...
        foreach (CodeElement childElement in currentElement.GetChildElements(innerOnly))
        {
    ...
    protected static void InlineParentClasses(CodeElement currentElement, CodeElement parent)
    {
        ...
        {
            foreach (CodeClass currentParent in parentClass.GetInheritanceTree())
            {
                foreach (CodeProperty? p in currentParent
                    .Properties
                    .Where(pp =>
    ...
    protected static void ReplaceLocalMethodsByGlobalFunctions(CodeElement currentElement, Func<CodeMethod, string> nameUpdateCallback, Func<CodeMethod, CodeUsing[]>? usingsCallback, params CodeMethodKind[] kindsToReplace)
    {
        ...
        {
            CodeUsing[]? usings = usingsCallback?.Invoke(currentMethod);
            string newName = nameUpdateCallback.Invoke(currentMethod);
            parentClass.RemoveChildElement(currentMethod);
            CodeFunction globalFunction = new(currentMethod)
            {
                Name = newName,
            };
            if (usings != null)
    ...
    {
        string staticMethodName = functionNameCallback.Invoke(returnType);
    ...
    private static void SetTypeAndAddUsing(CodeInterface inter, CodeType elemType, CodeElement targetElement)
    {
        ...
        CodeNamespace interNS = inter.GetImmediateParentOfType<CodeNamespace>();
        if (interNS != targetElement.GetImmediateParentOfType<CodeNamespace>())
        {
            CodeClass targetClass = targetElement.GetImmediateParentOfType<CodeClass>();
    ...
    {
        string interfaceName = interfaceNamingCallback.Invoke(modelClass);
        CodeNamespace targetNS = modelClass.GetImmediateParentOfType<CodeNamespace>();
        if (targetNS.FindChildByName<CodeInterface>(interfaceName, false) is { } existing)
        {
            return existing;
        }

        CodeClass? parentClass = modelClass.Parent as CodeClass;
        CodeInterface insertValue = new()
        {
            Name = interfaceName,
        ...
        {
            CodeInterface parentInterface = CopyClassAsInterface(baseClass, interfaceNamingCallback);
            inter.StartBlock.AddImplements(new CodeType
            {
                Name = parentInterface.Name,
                TypeDefinition = parentInterface,
            });
            CodeNamespace parentInterfaceNS = parentInterface.GetImmediateParentOfType<CodeNamespace>();
        ...
        {
            CodeType[] originalImplements = modelClass.StartBlock.Implements.Where(x => x.TypeDefinition != inter).ToArray();
        ...
        CodeElement[]? classModelChildItems = modelClass.GetChildElements(true)?.ToArray();
        if (classModelChildItems is not null)
        {
            foreach (CodeMethod? method in classModelChildItems.OfType<CodeMethod>()
                                                                    ...
                                                                    !(x.AccessedProperty?.IsOfKind(CodePropertyKind.AdditionalData) ?? false)))
            {
                    ...
                    !methodReturnType.IsExternal)
                {
                    ...
                    {
                        CodeUsing resultType = ReplaceTypeByInterfaceType(methodTypeClass, methodReturnType, usingsToRemove, interfaceNamingCallback);
                ...
                foreach (CodeParameter parameter in method.Parameters)
                {
                        ...
                        !parameterType.IsExternal)
                    {
                        ...
                        {
                            CodeUsing resultType = ReplaceTypeByInterfaceType(parameterTypeClass, parameterType, usingsToRemove, interfaceNamingCallback);
                            modelClass.AddUsing(resultType);
                            if (resultType.Declaration?.TypeDefinition?.GetImmediateParentOfType<CodeNamespace>() != targetNS)
                            {
                                targetUsingBlock.AddUsings((CodeUsing)resultType.Clone());
            ...
            foreach (CodeProperty mProp in classModelChildItems.OfType<CodeProperty>())
            {
        ...
        HashSet<string> externalTypesOnInter = inter.Methods.Select(static x => x.ReturnType).OfType<CodeType>().Where(static x => x.IsExternal)
    ...
    {
        CodeInterface propertyInterfaceType = CopyClassAsInterface(sourceClass, interfaceNamingCallback);
    ...
    public void AddQueryParameterMapperMethod(CodeElement currentElement, string methodName = "getQueryParameter", string parameterName = "originalName")
    {
        ...
        {
            CodeMethod method = currentClass.AddMethod(new CodeMethod
            {
                Name = methodName,
                Access = AccessModifier.Public,
                ReturnType = new CodeType
    ...
    protected void RemoveDiscriminatorMappingsTargetingSubNamespaces(CodeElement currentElement)
    {
        ...
        {
            CodeNamespace currentNamespace = currentMethod.GetImmediateParentOfType<CodeNamespace>();
            string[] keysToRemove = currentClass.DiscriminatorInformation.DiscriminatorMappings
    ...
    protected static void MoveRequestBuilderPropertiesToBaseType(CodeElement currentElement, CodeUsing baseTypeUsing, AccessModifier? accessModifier = null, bool addCurrentTypeAsGenericTypeParameter = false)
    {
        ...
        if (currentElement is CodeClass currentClass && currentClass.IsOfKind(CodeClassKind.RequestBuilder))
        {
            ...
            IEnumerable<CodeProperty> properties = currentClass.Properties.Where(static x => x.IsOfKind(CodePropertyKind.PathParameters, CodePropertyKind.UrlTemplate, CodePropertyKind.RequestAdapter));
    ...
    protected static void RemoveUntypedNodeTypeValues(CodeElement currentElement)
    {
        switch (currentElement)
        {
            case CodeProperty currentProperty when currentElement.Parent is CodeClass parentClass && currentProperty.Type.Name.Equals(ReQuestyBuilder.UntypedNodeName, StringComparison.OrdinalIgnoreCase):
                parentClass.RemoveChildElement(currentProperty);
                break;
            case CodeMethod currentMethod when currentMethod.IsOfKind(CodeMethodKind.RequestExecutor):
                if (currentMethod.ReturnType.Name.Equals(ReQuestyBuilder.UntypedNodeName, StringComparison.OrdinalIgnoreCase))
                {
                    currentMethod.ReturnType = new CodeType { Name = "binary", IsExternal = true };
                }
                if (currentMethod.Parameters.Where(x => x.Kind is CodeParameterKind.RequestBody && x.Type.Name.Equals(ReQuestyBuilder.UntypedNodeName, StringComparison.OrdinalIgnoreCase)).ToList() is { Count: > 0 } parameters)
                {
    ...
    protected static void RemoveRequestConfigurationClasses(CodeElement currentElement, CodeUsing? configurationParameterTypeUsing = null, CodeType? defaultValueForGenericTypeParam = null, bool keepRequestConfigurationClass = false, bool addDeprecation = false, CodeUsing? usingForDefaultGenericParameter = null)
    {
            ...
            currentClass.Parent is CodeClass parentClass)
        {
            ...
            if (configurationParameterTypeUsing != null && genericTypeParamValue != null && configurationParameters.Length != 0)
            {
                ...
                CodeType configurationParameterType = new()
                {
