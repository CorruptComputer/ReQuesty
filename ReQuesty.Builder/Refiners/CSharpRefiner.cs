...
public class CSharpRefiner(GenerationConfiguration configuration) : CommonLanguageRefiner(configuration), ILanguageRefiner
{
        ...
        return Task.Run(() =>
        {
            ...
            */
            ReplaceReservedNames(
                ...
                [typeof(CodeClass), typeof(ClassDeclaration), typeof(CodeProperty), typeof(CodeUsing), typeof(CodeNamespace), typeof(CodeMethod), typeof(CodeEnum), typeof(CodeEnumOption)]
            );
            ReplaceReservedNames(
                generatedCode,
    ...
    protected static readonly AdditionalUsingEvaluator[] defaultUsingEvaluators = [
        ...
        new (static x => x is CodeProperty prop && prop.IsOfKind(CodePropertyKind.Custom) && prop.Type.Name.Equals(ReQuestyBuilder.UntypedNodeName, StringComparison.OrdinalIgnoreCase),
            SerializationNamespaceName, ReQuestyBuilder.UntypedNodeName),
        new (static x => x is CodeMethod @method && @method.IsOfKind(CodeMethodKind.RequestExecutor) && (method.ReturnType.Name.Equals(ReQuestyBuilder.UntypedNodeName, StringComparison.OrdinalIgnoreCase) ||
                                                                                                        method.Parameters.Any(x => x.Kind is CodeParameterKind.RequestBody && x.Type.Name.Equals(ReQuestyBuilder.UntypedNodeName, StringComparison.OrdinalIgnoreCase))),
            SerializationNamespaceName, ReQuestyBuilder.UntypedNodeName),
    ...
    protected static void CorrectMethodType(CodeMethod currentMethod)
    {
        ...
        CorrectCoreTypes(currentMethod.Parent as CodeClass, DateTypesReplacements, true, currentMethod.Parameters
                                                .Select(x => x.Type)
                                                .Union([currentMethod.ReturnType])
                                                .ToArray());
    }
