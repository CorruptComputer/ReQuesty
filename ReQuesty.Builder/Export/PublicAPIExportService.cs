...
namespace ReQuesty.Builder.Export;
...
internal class PublicApiExportService
{
    ...
    private readonly CSharpConventionService _conventionService;
    internal const string DomExportFileName = "requesty-dom-export.txt";
    ...
    {
        StreamWriter streamWriter = new(outputStream, leaveOpen: true);
        await using (streamWriter.ConfigureAwait(false))
        {
            string[] entries = GetEntriesFromDom(rootNamespace).Order(StringComparer.OrdinalIgnoreCase).ToArray();
    ...
    {
        foreach (string? currentElementEntry in GetEntry(currentElement).Where(static x => !string.IsNullOrEmpty(x)))
        {
            yield return currentElementEntry;
        }

        foreach (CodeElement childElement in currentElement.GetChildElements())
        {
            foreach (string childElementEntry in GetEntriesFromDom(childElement))
            {
    ...
    private static CSharpConventionService GetLanguageConventionServiceFromConfiguration(GenerationConfiguration generationConfiguration)
    {
