...
public class CodeMethod : CodeTerminalWithKind<CodeMethodKind>, ICloneable, IDocumentedElement, IDeprecableElement, IAccessibleElement
{
    ...
    public void RemoveParametersByKind(params CodeParameterKind[] kinds)
    {
        parameters.Where(p => p.Value.IsOfKind(kinds))
                            ...
                            .ToList()
                            .ForEach(x => parameters.Remove(x, out CodeParameter? _));
    }
    ...
    {
        if (!errorMappings.TryGetValue(ErrorMappingClientRange, out CodeTypeBase? clientError) || !errorMappings.TryGetValue(ErrorMappingServerRange, out CodeTypeBase? serverError))
        {
            return;
        }
        ...
        {
            errorMappings.TryRemove(ErrorMappingServerRange, out CodeTypeBase? _);
    ...
    {
        string[] codes = errorMappings.Where(x => x.Value == oldType).Select(static x => x.Key).ToArray();
