...
public class CodeBlock<TBlockDeclaration, TBlockEnd> : CodeElement, IBlock where TBlockDeclaration : BlockDeclaration, new() where TBlockEnd : BlockEnd, new()
{
    ...
    public virtual void RenameChildElement(string oldName, string newName)
    {
        ...
        {
            throw new InvalidOperationException($"The element to rename was not found {oldName}");
        }
    }
    ...
    protected IEnumerable<T> AddRange<T>(params T[] elements) where T : CodeElement
    {
        ...
        T[] result = new T[elements.Length]; // not using yield return as they'll only get called if the result is assigned

        for (int i = 0; i < elements.Length; i++)
        {
            T element = elements[i];
            CodeElement returnedValue = InnerChildElements.GetOrAdd(element.Name, element);
    ...
    private T HandleDuplicatedExceptions<T>(T element, CodeElement returnedValue) where T : CodeElement
    {
        ...
        if (element is CodeMethod currentMethod)
        {
            ...
            {
                HashSet<string> currentMethodParameterNames = currentMethod.Parameters.Select(static x => x.Name).ToHashSet();
                HashSet<string> returnedMethodParameterNames = existingMethod.Parameters.Select(static x => x.Name).ToHashSet();
                                                ...
                                                .Any())
                {
                    ...
                    string methodOverloadNameSuffix = currentMethodParameterNames.Count != 0 ? currentMethodParameterNames.OrderBy(static x => x).Aggregate(static (x, y) => x + y) : "1";
    ...
    public T? FindChildByName<T>(string childName, uint maxDepth) where T : ICodeElement
    {
        ...
        if (InnerChildElements.TryGetValue(childName, out CodeElement? result) && result is T castResult)
        {
            return castResult;
        }
...
public class BlockDeclaration : CodeTerminal
{
    ...
    public void AddUsings(params CodeUsing[] codeUsings)
    {
        ...
        foreach (CodeUsing codeUsing in codeUsings)
        {
    ...
    public void RemoveUsings(params CodeUsing[] codeUsings)
    {
        ...
        foreach (CodeUsing codeUsing in codeUsings)
        {
            usings.TryRemove(codeUsing, out bool _);
        }
    }
    public void RemoveUsingsByDeclarationName(params string[] names)
    {
        ...
        HashSet<string> namesAsHashSet = names.ToHashSet(StringComparer.OrdinalIgnoreCase);
        foreach (CodeUsing? usingToRemove in usings.Keys.Where(x => !string.IsNullOrEmpty(x.Declaration?.Name) && namesAsHashSet.Contains(x.Declaration!.Name)))
        {
            usings.TryRemove(usingToRemove, out bool _);
        }
    }
}
