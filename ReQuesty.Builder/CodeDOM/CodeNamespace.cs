...
public class CodeNamespace : CodeBlock<BlockDeclaration, BlockEnd>
{
    ...
    {
        CodeFile file = FindChildByName<CodeFile>(fileName, false) ?? new CodeFile { Name = fileName };
    ...
    {
        if (Parent is CodeNamespace parentNS)
        {
            return parentNS.GetRootNamespace();
        }
    ...
    public CodeNamespace? FindNamespaceByName(string nsName, bool findInChildElements = false)
    {
        ...
        if (nsName.Equals(Name, StringComparison.OrdinalIgnoreCase))
        {
            return this;
        }
    ...
    public CodeNamespace AddNamespace(string namespaceName)
    {
        ...
        string[] namespaceNameSegments = namespaceName.Split(NamespaceNameSeparator, StringSplitOptions.RemoveEmptyEntries);
        int lastPresentSegmentIndex = default(int);
        CodeNamespace? lastPresentSegmentNamespace = GetRootNamespace();
        while (lastPresentSegmentIndex < namespaceNameSegments.Length)
        {
            CodeNamespace? segmentNameSpace = lastPresentSegmentNamespace.FindNamespaceByName(namespaceNameSegments.Take(lastPresentSegmentIndex + 1).Aggregate(static (x, y) => $"{x}.{y}"));
        ...
        foreach (string? childSegment in namespaceNameSegments.Skip(lastPresentSegmentIndex))
        {
    ...
    public NamespaceDifferentialTracker GetDifferential(CodeNamespace importNamespace, string namespacePrefix, char separator = '.')
    {
        ...
        string[] currentNamespaceSegments = Name[prefixLength..]
                                .Split(separator, StringSplitOptions.RemoveEmptyEntries);
        string[] importNamespaceSegments = importNamespace
                            .Name[prefixLength..]
                            .Split(separator, StringSplitOptions.RemoveEmptyEntries);
        int importNamespaceSegmentsCount = importNamespaceSegments.Length;
        int currentNamespaceSegmentsCount = currentNamespaceSegments.Length;
        int deeperMostSegmentIndex = 0;
        ...
        int upMoves = currentNamespaceSegmentsCount - deeperMostSegmentIndex;
        return new()
