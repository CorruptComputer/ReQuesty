...
public class DiscriminatorInformation : CodeElement, ICloneable
{
    ...
    {
        ArgumentException.ThrowIfNullOrEmpty(key);
        if (discriminatorMappings.TryGetValue(key, out CodeType? value))
        {
            return value;
        }
    ...
    public void RemoveDiscriminatorMapping(params string[] keys)
    {
        ...
        {
            discriminatorMappings.TryRemove(key, out CodeType? _);
        }
    }
    ...
    public void RemoveDiscriminatorMapping(CodeClass classToRemove)
    {
        ...
        string keyToRemove = discriminatorMappings.FirstOrDefault(x => x.Value is CodeType currentType && currentType.TypeDefinition == classToRemove).Key;
        if (!string.IsNullOrEmpty(keyToRemove))
        {
            discriminatorMappings.TryRemove(keyToRemove, out CodeType? _);
        }
    }
