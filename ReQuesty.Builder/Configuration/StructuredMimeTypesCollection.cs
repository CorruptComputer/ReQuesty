...
public partial class StructuredMimeTypesCollection : ICollection<string>
{
    ...
    {
        if (!string.IsNullOrEmpty(rawFormat) && MediaTypeHeaderValue.TryParse(rawFormat, out MediaTypeHeaderValue? parsedFormat) && parsedFormat.MediaType is not null)
        {
            float priority = parsedFormat.Parameters.FirstOrDefault(static x => isPriorityParameterName(x.Name)) is { } priorityParameter && float.TryParse(priorityParameter.Value, CultureInfo.InvariantCulture, out float resultPriority) ? resultPriority : 1;
    ...
    {
        string additionalParameters = string.Join(";", value.Parameters.Where(static x => !isPriorityParameterName(x.Name)).Select(static x => $"{x.Name}={x.Value}"));
        string mediaType = string.IsNullOrEmpty(value.MediaType) ? "*/*" : value.MediaType;
    ...
    public float? GetPriority(string mimeType)
    {
        ...
        return TryGetMimeType(mimeType, out float priority) ? priority : null;
    }
    ...
    public IEnumerable<string> GetAcceptedTypes(IEnumerable<string> searchTypes)
    {
        ...
        return searchTypes.Select(GetKeyAndPriority)
                        ...
                        .Select(x => TryGetMimeType(x, out float result) ? NormalizeMimeType(x, result) : null)
                        .OfType<string>()
    ...
    public IEnumerable<string> GetContentTypes(IEnumerable<string> searchTypes)
    {
        ...
        return searchTypes.Select(GetKeyAndPriority)
                        ...
                        .Select(static x => x.Key)
                        .Distinct(StringComparer.OrdinalIgnoreCase)
                        .Select(x => TryGetMimeType(x, out float result) ? new KeyValuePair<string, float>?(new(x, result)) : null)
                        .OfType<KeyValuePair<string, float>>()
