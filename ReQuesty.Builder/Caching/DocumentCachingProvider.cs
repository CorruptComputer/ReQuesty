...
namespace ReQuesty.Builder.Caching;
...
public class DocumentCachingProvider
{
    ...
    {
        string hashedUrl = Convert.ToHexString((HashAlgorithm.Value ?? throw new InvalidOperationException("unable to get hash algorithm")).ComputeHash(Encoding.UTF8.GetBytes(documentUri.ToString()))).Replace("-", string.Empty, StringComparison.OrdinalIgnoreCase);
        string target = Path.Combine(Path.GetTempPath(), Constants.TempDirectoryName, "cache", intermediateFolderName, hashedUrl, fileName);
        ...
        {// if multiple clients are being updated for the same description, we'll have concurrent download of the file without the lock
            ...
            DateTime lastModificationDate = File.GetLastWriteTime(target);
    ...
    private async Task<Stream> DownloadDocumentFromSourceAsync(Uri documentUri, string target, string? accept, CancellationToken token)
    {
        ...
        try
        {
            using HttpRequestMessage requestMessage = new(HttpMethod.Get, documentUri);
            if (!string.IsNullOrEmpty(accept))
            {
                requestMessage.Headers.Add("Accept", accept);
            }

            using HttpResponseMessage responseMessage = await HttpClient.SendAsync(requestMessage, token).ConfigureAwait(false);
