...
internal class OpenApiSchemaComparer(
    OpenApiDiscriminatorComparer? discriminatorComparer = null,
    JsonNodeComparer? jsonNodeComparer = null) : IEqualityComparer<IOpenApiSchema>
{
    private readonly OpenApiDiscriminatorComparer discriminatorComparer = discriminatorComparer ?? new OpenApiDiscriminatorComparer();
    private readonly JsonNodeComparer jsonNodeComparer = jsonNodeComparer ?? new JsonNodeComparer();
    ...
    private void GetHashCodeInternal([DisallowNull] IOpenApiSchema obj, HashSet<IOpenApiSchema> visitedSchemas, ref HashCode hash)
    {
        ...
        {
            foreach (KeyValuePair<string, IOpenApiSchema> prop in obj.Properties)
            {
        ...
        {
            foreach (IOpenApiSchema schema in obj.OneOf)
            {
        ...
        {
            foreach (IOpenApiSchema schema in obj.AnyOf)
            {
                GetHashCodeInternal(schema, visitedSchemas, ref hash);
            }
        }
        ...
        {
            foreach (IOpenApiSchema schema in obj.AllOf)
            {
                GetHashCodeInternal(schema, visitedSchemas, ref hash);
            }
        }
...
internal class OpenApiDiscriminatorComparer : IEqualityComparer<OpenApiDiscriminator>
{
    ...
    public int GetHashCode([DisallowNull] OpenApiDiscriminator obj)
    {
        ...
        foreach (KeyValuePair<string, string> mapping in GetOrderedRequests(obj.Mapping))
        {
